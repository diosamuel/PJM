      ...
      <header className="bg-white fixed top-0 z-30 w-full shadow-md border-b-4 border-white hidden lg:block">
        <div className="flex justify-between mx-10">
          ...
          <div className="flex">
            ...
            <button className="w-100 bg-white border-2 border-orange-500 text-orange-500 transition m-3 p-2 px-3 rounded-full font-semibold text-sm hover:bg-orange-500 hover:text-white" onClick={jualBakModal}>JUAL BAK ANDA <i className="fa-solid fa-plus"></i></button>
          </div>
          ...
          <div className="flex items-center gap-4">
            <Link href="/toko/keranjang" className="text-center text-black hover:text-primary transition relative">
              ...
              <div
                className="absolute right-0 -top-1 w-5 h-5 rounded-full flex items-center justify-center bg-blue-600 text-white text-sm">
                {cart.length || 0}</div>
            </Link>
            <Link href={`${process.env.NEXT_PUBLIC_API_DASHBOARD}/login`} className="text-center text-black hover:text-primary transition relative hidden lg:block">
              <div className="text-2xl">
      ...
      <header className="bg-white fixed top-0 z-30 w-full shadow-md border-b-4 border-white px-2 text-center lg:hidden">
        <div className="flex p-2 gap-3 items-center w-full">
          ...
          <Link href="/toko/keranjang" className="text-center text-black hover:text-primary transition w-1/12 relative">
            ...
            <span className="relative -right-2 -top-2 w-5 h-5 rounded-full flex items-center justify-center bg-blue-600 text-white text-sm">{cart.length || 0}</span>
          </Link>
        </div>
      </header>
